...
const EnterpriseProductGrid: React.FC<ProductGridProps> = memo(({
  ...
  // ===== EMPTY STATE =====
  if (!products.length) {
    console.log('[EnterpriseProductGrid] No products provided, showing empty state');
    return (
      <div className={`w-full ${className}`} data-testid="product-grid-empty">
        <div className="text-center py-12">
          ...
          <div className="text-sm text-gray-500">
            Products received: {products?.length || 0}
          </div>
        </div>
      </div>
    );
  ...
  // ===== MAIN RENDER =====
  console.log('[EnterpriseProductGrid] Rendering grid with products:', products?.length || 0);
  console.log('[EnterpriseProductGrid] Sorted products:', sortedProducts?.length || 0);
