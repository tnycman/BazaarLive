// Fashion Category Configuration - Enterprise-grade category management
import { 
  FashionCategory, 
  FASHION_CATEGORIES, 
  FASHION_CATEGORY_LABELS,
  ProductCondition,
  FashionColor,
  FashionMaterial 
} from "../types/FashionDomain";

// Subcategory Configuration Interface
...
export interface SizeOption {
  value: string;
  label: string;
  system: 'us' | 'uk' | 'eu' | 'international' | 'numeric' | 'freesize';
...
const WOMENS_CONFIG: CategoryConfig = {
  subcategories: [
    {
      value: 'dresses',
      label: 'Dresses',
...
const MENS_CONFIG: CategoryConfig = {
  subcategories: [
    {
      value: 'shirts',
      label: 'Shirts',
...
const KIDS_CONFIG: CategoryConfig = {
  subcategories: [
    {
      value: 'clothing',
      label: 'Clothing',
...
export const FASHION_CATEGORY_CONFIG: Record<FashionCategory, CategoryConfig> = {
  women: WOMENS_CONFIG,
  men: MENS_CONFIG,
  kids: KIDS_CONFIG,
  home: {
...
export function getCategoryConfig(category: FashionCategory): CategoryConfig {
  return FASHION_CATEGORY_CONFIG[category];
}

export function getSubcategories(category: FashionCategory): SubcategoryOption[] {
  return getCategoryConfig(category).subcategories;
}

export function getSizesForCategory(category: FashionCategory, subcategory?: string): SizeOption[] {
...
export function getPopularBrandsForCategory(category: FashionCategory): BrandOption[] {
  return getCategoryConfig(category).popularBrands.filter(brand => brand.isPopular);
}

export function getAttributesForCategory(category: FashionCategory): CategoryAttribute[] {
  return getCategoryConfig(category).attributes;
}

export function validateCategorySubcategory(category: FashionCategory, subcategory: string): boolean {
  const config = getCategoryConfig(category);
  return config.subcategories.some(sub => sub.value === subcategory);
}

export function getSearchTermsForSubcategory(category: FashionCategory, subcategory: string): string[] {
  const config = getCategoryConfig(category);
  const subCat = config.subcategories.find(sub => sub.value === subcategory);
  return subCat?.searchTerms || [];
}

export function getCategorySearchBoosts(category: FashionCategory): SearchBoost[] {
